...
class StreamDash:
	...
	def render_sidebar(well:Well):
		...
		# Well basic info
		st.sidebar.text_input("Well Name", value=well.name, disabled=True)
		st.sidebar.text_input("Field", value=well.field, disabled=True)
		st.sidebar.text_input("Operator", value=well.operator, disabled=True)
	...
	def render_construction_tab(self):
	...
	def render_fluid_analysis_tab(self):
	...
	def run(self):
		...
		with tab5:
			self.render_fluid_analysis_tab()