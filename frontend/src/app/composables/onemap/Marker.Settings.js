const DEFAULT_MODAL_ID = "marker-settings-modal";
...
function addListener(cleanup, target, type, handler, options) {
  if (!target?.addEventListener) return;
  target.addEventListener(type, handler, options);
  cleanup.push(() => target.removeEventListener(type, handler, options));
}

export function initMarkerSettings({ modalId = DEFAULT_MODAL_ID } = {}) {
  ...
  if (!panelEl || !selectEl || !placeholderEl || !colorListEl || !resetBtn) {
    return null;
  }
  ...
  function ensureColorMapping(columnInfo) {
    ...
    columnInfo.values.forEach((value) => {
      ...
      const fallback =
        palette[paletteIndex % palette.length] || modalState.defaultBaseColor;
      paletteIndex += 1;
      colors[key] = fallback;
    });
  ...
  function updateDefaultColorFromMap() {
    ...
    if (
      colorState &&
      typeof colorState.defaultColor === "string" &&
      colorState.defaultColor.trim()
    ) {
  ...
  function refreshColumns() {
    ...
    const info =
      typeof window.getCategoricalColumnsInfo === "function"
  ...
  function handleResetClick(event) {
    const trigger =
      event.target instanceof Element
        ? event.target.closest('[data-role="marker-reset"]')
        : null;
    if (!trigger) return;
    event.preventDefault();
    if (!modalState.activeColumn) return;
    const columnInfo = modalState.columns.find(
      (col) => col.name === modalState.activeColumn
    );
    ...
    columnInfo.values.forEach((value, index) => {
      resetMap[value.key] =
        palette[index % palette.length] || modalState.defaultBaseColor;
    });
  ...
  function showWithBootstrapLegacy() {
  ...
  addListener(cleanup, selectEl, "change", handleColumnChange);
  addListener(cleanup, panelEl, "input", handleColorInput);
  addListener(cleanup, panelEl, "click", handleResetClick);
  addListener(cleanup, document, "efs:dataset-ready", refreshColumns);

  addListener(cleanup, document, "keydown", (event) => {
  ...
  modalEl.querySelectorAll('[data-dismiss="modal"]').forEach((el) => {
    addListener(cleanup, el, "click", (event) => {
      if (activeModalMode === "fallback") {
        event.preventDefault();
        hideFallback();
      }
  ...
  addListener(cleanup, modalEl, "hide.bs.modal", () => {
