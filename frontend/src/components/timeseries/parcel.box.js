...
function createBoxTemplate(index, templateRoot) {
    const parcelTmp = resolveTemplate(templateRoot, "parcel-box");
    if (!parcelTmp?.content) return null;
    ...
    const addTmp = resolveTemplate(templateRoot, "add-button");
    if (addTmp?.content && boxHeader) {
        const add = addTmp.content.cloneNode(true);
        const addButton = add.querySelector(".add-button");
        if (addButton) {
            boxHeader.appendChild(addButton);
        }
    }

    const setTmp = resolveTemplate(templateRoot, "settings-button");
    if (setTmp?.content && boxHeader) {
        const set = setTmp.content.cloneNode(true);
        const settingsButton = set.querySelector(".settings-circle");
        if (settingsButton) {
            boxHeader.appendChild(settingsButton);
        }
    }
...
function cleanupBoxResources(box, { remove = true } = {}) {
    if (!box) return;
    box.seriesPicker?.destroy?.();
    box.traceStyler?.destroy?.();
    box.fig?.resizeObserver?.disconnect();
    if (box.fig) {
        box.fig.resizeObserver = null;
    }
    if (box.plotDiv) {
        try {
...
function initTimeseriesBox(
    ...
    if (!box.id && (index || index === 0)) {
        box.id = `box-${index}`;
    }

    box.useDomPills = useDomPills;
    box.fig = createPlotSchema();
    box.plotDiv = box.querySelector(".plot-display");
    if (!box.plotDiv) return null;

    const ro = new ResizeObserver(() => {
        Plotly.Plots.resize(box.plotDiv);
    });

    ro.observe(box.plotDiv.parentElement || box);
    box.fig.resizeObserver = ro;

    const normalizedFields = Array.isArray(fields) ? fields : [];
    box.fields = normalizedFields;
    ...
    if (enablePicker) {
        box.seriesPicker = createSeriesPicker(box, {
            getFigure: () => box.fig,
            getTraceData: (key) => getTraceData(box.data, key),
            setTrace,
            removeTrace,
            fields: box.fields,
        ...
        if (box.seriesPicker?.lastAggs) {
            box.lastAggs = box.seriesPicker.lastAggs;
        }
        if (normalizedFields.length) {
            box.seriesPicker?.fillChoices?.(normalizedFields);
        }
    }

    if (enableStyler) {
        box.traceStyler = createTraceStyler(box, { getFigure: () => box.fig, templateRoot });
    }

    refreshPlot(box);

    box.setFields = (nextFields) => {
        const next = Array.isArray(nextFields) ? nextFields : [];
        box.fields = next;
        box.seriesPicker?.fillChoices?.(next);
    };

    box.refreshData = async (filters = {}) => {
        if (!box.fig) return;
        const aggs = box.seriesPicker?.lastAggs || box.lastAggs || {};
        try {
    ...
    box.destroy = () => {
        cleanupBoxResources(box, { remove: false });
    };
...
function createBox(
    ...
    const box = createBoxTemplate(index, templateRoot);
    if (!box) return null;
    host.insertBefore(box, host.lastElementChild);

    return initTimeseriesBox(box, {
        fields,
        onAggSelectionsChange,
        index,
        templateRoot,
