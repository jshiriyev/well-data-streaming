export function createFilterSidebar({
	...
	const noop = () => {};
	if (!host) {
		return { setFields: noop, getEffectiveFilters: () => ({}), updateFilterPill: noop };
	}
	...
	const pill = buildFilterPill(pillHost);
...
let pillListenersAttached = false;

function buildFilterPill(pillHost) {
	if (!pillHost) return null;

	const existingChip = pillHost.querySelector(".filter-chip");
	if (!existingChip) {
		const temp = document.getElementById("filter-pill");
		if (temp?.content) {
	...
	if (chip && dropdown && !pillListenersAttached) {
		const closeDropdown = () => {
		...
		pillListenersAttached = true;
	}

	return { chip, chipLabel, dropdown, list };
}
