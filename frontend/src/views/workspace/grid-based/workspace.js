(() => {
    ...
    function addToDock(panel) {
        ...
        const pill = document.createElement("div"); pill.className = "dock-pill"; pill.dataset.dockFor = panel.id;
        const title = panel.querySelector(".panel-title span").textContent || panel.id;
        pill.innerHTML = `
        <span>${escapeHtml(title)}</span>
        <span class="mini-x" title="Close">├ù</span>
        `;
    ...
    function removeFromDock(panel) { const pill = dockItems.querySelector(`[data-dock-for="${panel.id}"]`); if (pill) pill.remove(); } function minimizeToDock(panel) {         // (4) can't minimize if maximized         if (panel.dataset.maximized === "true") return;
        panel.dataset.minimized = "true"; addToDock(panel);
    }

    function restoreFromDock(panel) {
        panel.dataset.minimized = "false"; removeFromDock(panel);         // ensure inside bounds         const c = getCanvasRect();         const rect = panel.getBoundingClientRect();         const w = rect.width;         const h = rect.height;         const left = clamp(parseFloat(panel.style.left || "12"), 0, c.width - w);         const top = clamp(parseFloat(panel.style.top || "12"), 0, c.height - h);         panel.style.left = `${left}px`;         panel.style.top = `${top}px`;         canvas.appendChild(panel);         focusPanel(panel);     }      // ----- Panel creation -----     function createPanel({ title = "Panel", body = "" } = {}) {         panelCount += 1;         const id = `panel-${panelCount}`;
        const panel = document.createElement("section"); panel.className = "panel"; panel.id = id;
        const c = getCanvasRect(); const left = snap(24 + (panelCount * 18) % Math.max(1, (c.width - 460))); const top = snap(24 + (panelCount * 14) % Math.max(1, (c.height - 320))); panel.style.left = `${left}px`; panel.style.top = `${top}px`; panel.style.zIndex = String(++zTop); panel.dataset.minimized = "false"; panel.dataset.maximized = "false"; panel.dataset.focused = "false";
        panel.innerHTML = `             <header class="panel-header" data-drag-handle="true">               <div class="panel-title">                 <span>${escapeHtml(title)}</span>                 <span class="panel-badge">#${panelCount}</span>               </div>               <div class="panel-controls">                 <button class="icon-btn" title="Minimize" data-action="minimize">ΓÇô</button>                 <button class="icon-btn" title="Maximize" data-action="maximize">Γûí</button>                 <button class="icon-btn" title="Close" data-action="close">├ù</button>               </div>             </header>             <div class="panel-body">               <div class="placeholder">                 ${body || "Empty panel. Replace this with Plotly, tables, forms, logs, etc."}               </div>             </div>             <div class="resize-handle" data-resize-handle="true" title="Resize"></div>           `;
        panel._restore = null;
        // focus
        ...
        enableDrag(panel); enableResize(panel);
        canvas.appendChild(panel); focusPanel(panel); syncMinimizeDisabled(panel); return panel;
    } function syncMinimizeDisabled(panel) {
        const minBtn = panel.querySelector('button[data-action="minimize"]'); const isMax = panel.dataset.maximized === "true"; minBtn.disabled = isMax; // (4)         minBtn.title = isMax ? "Cannot minimize while maximized" : "Minimize";     }      function toggleMaximize(panel) {         const isMax = panel.dataset.maximized === "true";
        ...
        } focusPanel(panel); syncMinimizeDisabled(panel);
    }
    ...
    function optimizePanels() {
        ...
        } const c = getCanvasRect(); const gap = 0; const n = panels.length;
        // choose cols/rows to approach aspect ratio         let cols = Math.ceil(Math.sqrt(n));         let rows = Math.ceil(n / cols);
        const cellW = Math.floor((c.width - gap * (cols + 1)) / cols); const cellH = Math.floor((c.height - gap * (rows + 1)) / rows);
        panels.forEach((p, i) => {
    ...
    } gridSizeSel.addEventListener("change", () => { setGridSizeCss(); });
    // ----- Buttons -----
    ...
    } const gridModeToggle = document.getElementById("gridModeToggle"); gridModeToggle.addEventListener("click", function () {
        gridModeToggle.classList.toggle('on');
        const knobElement = gridModeToggle.querySelector(".knob")
        if (knobElement) { knobElement.classList.toggle('on'); }
        gridModeOn = !gridModeOn; gridBg.classList.toggle("on", gridModeOn);
    }); const gridSnapToggle = document.getElementById("gridSnapToggle"); gridSnapToggle.addEventListener("click", function () {
