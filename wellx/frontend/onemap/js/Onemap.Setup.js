...
// const latlng = L.latLng(40.395, 50.035);

const initialDate = new Date(SessionState.time); // adjust var name if needed
...
var bounds = [[40.18237924998362, 51.01761966086437], [40.25439042443973, 51.1234682493391]];

var fieldContours = L.tileLayer('/onemap/tile/{z}/{x}/{y}.png', {
	tms: true,
	name: 'FLD Contours',
	overlay: true,
	opacity: 1.0,
...
const filterSidebar = L.control.sidebar('sidebar-filter', {
	position: 'left',
	autoPan: true,
	closeButton: false,
}).addTo(map);
...
function createHorizonControl(horizons = []) {
	...
	horizons.forEach(item => {
		...
		li.addEventListener('click', () => {
			...
			// simple visual indication of selected item
			Array.from(menu.children).forEach(li => li.style.fontWeight = 'normal');
			li.style.fontWeight = 'bold';

			button.classList.remove('open');
			menu.classList.remove('show');

			// hook: do something when horizon changes
			// console.log('Selected horizon:', selected);
...
const HorizonControl = L.Control.extend({
	...
	onAdd: function () {
		this._container = createHorizonControl(this._horizons);
...
var timeControl = new L.Control.TimeDimension({
    ...
    minSpeed: 1,
    speedStep: 1,
	timeSliderDragUpdate: true, // (optional but nice UX)
	// timeFormat: "custom",
	// timeDisplayFormat: "YYYY-MM-DD",
	// // displayDateFormat: "YYYY-MM-DD",
	// timeDisplayCallback: function(date) {
    //     // date comes as a Date object
...
map.timeDimension.on("timeload", function (e) {
	...
	// console.log("Time changed to:", SessionState.time);
...
const ensureBottomLeftColumn = () => {
	...
	const bottomLeftCorner = mapContainer.querySelector('.leaflet-bottom.leaflet-left');
	if (!bottomLeftCorner) return null;

	let columnContainer = bottomLeftCorner.querySelector('.leaflet-bottom-left-row');
	if (!columnContainer) {
...
const alignBottomLeftControls = () => {
	...
	if (!horizonReady || !timeReady) {
		requestAnimationFrame(alignBottomLeftControls);
	}
};
...
map.whenReady(() => {
	updateMap(wellPointFeatureGroup);
});
